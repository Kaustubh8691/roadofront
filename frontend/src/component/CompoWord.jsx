  ...
  return (
    ...
    <div>
        <div id="vocab-heading" >
          {(tri)?(<div className='head'><div id='vocab'>Vocab</div> <img onClick={()=>handlesearch()} src={img} alt="ajsh"/></div>
): <div className='serach-div'><input className='serach-input'placeholder='search' onChange={(e)=>dats(e.target.value)} autoFocus/><button className='cross' onClick={()=>{setTri(true);getItemsList()}}>x</button></div>}
          {/* <div className='head'><div id='vocab'>Vocab</div> <img onClick={()=>handlesearch()} src={img} alt="ajsh"/></div> */}
        ...
        <button id='button-add' onClick={()=>setTrim(true)}>+</button>
              <Add setTrim={setTrim} trim={trim}></Add>
        </div>
    </div>
    </>
  )
